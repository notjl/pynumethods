...
class Formula(object):
    ...
    def e(self):
        self.error = round(
            abs(((self.curr_Xn-self.prev_Xn)/self.curr_Xn)*100), 4)
    ...
    def wrapper_printer(self):
        if self.mode == '0':
            print(
                f'Since {self.header} %e is close to 0.0000%, stopping iteration')
...
def main():
    ...
    for i in range(formula_range):
        ...
        formulae.append(
            Formula(formula=f, n=n, header=f'{chr(ord("f")+i)}(x)'))
    ...
    try:
        ...
        for i in range(formula_range):
            if formula_range > 1:
                print(end=f'{formulae[i].root:.4f}, ' if formula_range -
                      1 != i else f'{formulae[i].root:.4f}\n\n')
